...
class SortedMultiset(Generic[T]):
    ...
    # s[x] : indexing 
    # s.index[x] : return the number of leafs less than x 
    # if x in s then returns the index of x 
...
def main():
    N = isint()
    l = sorted(list(ismap()))
    sorted_list = SortedMultiset(l)
    ab_list = list(combinations(l, 2))
    print(ab_list)
    ans = 0
    for i in range(len(ab_list)):
        ...
        largest_c = a + b - 1
        smallest_c = max(a, b)
        L_ind = bisect_right(L, largest_c)
        s_ind = bisect_left(L, smallest_c)
        ans += abs(L_ind - s_ind)
        print(a, b, L_ind - s_ind)
    return ans 
