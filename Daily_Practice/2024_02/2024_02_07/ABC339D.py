...
last_match = [[-1, -1] for _ in range(N)] 

A_sorted = sorted(A)

seq = {v: i for i, v in enumerate(A_sorted)}

st = SegTree([0 for _ in range(N)], mode="max")
for idx, val in enumerate(A):
    l = bisect_left(A_sorted, val-D)
    r = bisect_right(A_sorted, val+D)
    v = st.query(l, r) + 1
    st.update(seq[val], v)
