...
def main():
    ...
    A = list(ismap())
    ans = N
    dp = [[[(0) for _ in range(sum(A)+1)] for _ in range(N)] for _ in range(N+1)]
    dp[0][0][0] = 1 
    for i in range(N+1):
        for k in range(N):
            for s in range(sum(A)+1):
                if dp[i][k][s] == 0: continue 
                if (i+1) < N:
                    dp[i+1][k][s] += dp[i][k][s]
                if (k*s + A[i]) % (k+1) != 0: continue
                if (i+1) < N and (k+1) < N and ((k*s + A[i]) // (k+1)) <= N*sum(A):
                    dp[i+1][k+1][(k*s + A[i]) // (k+1)] += dp[i][k][s]
